...
public final class Tokenizer {
    ...
    private static String nextWordOrSeparator(String text, int position) {
        ...
        if (SEPARATORS.indexOf(text.charAt(position)) < 0) { //no separators appear in the char
    ...
    public static Queue<String> tokens(SimpleReader in) {
        ...
        Queue token = new Queue1L<>();
        while (!in.atEOS()) {
            String line = in.nextLine();
            int position = 0;
            String tokenString = "";
            while (position < line.length()) {
                tokenString = nextWordOrSeparator(line, position);
                position = position + tokenString.length();//Key step!! do not use token.length
                if (!SEPARATORS.contains(tokenString)) {
                    token.enqueue(tokenString);
                }
            }
        }
        token.enqueue(END_OF_INPUT);
